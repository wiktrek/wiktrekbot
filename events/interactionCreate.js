...
client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.isCommand()) {
        ...
        if (!cmd) {
            ...
            console.log(`this coommand doesnt exist! (error) in ${interaction.member.guild.name}`);
            }
            if (!cmd) return
        const args = [];
        ...
        if(!interaction.member.permissions.has(cmd.userPerm)) return interaction.followUp({ content: 'You don\'t have perms to do that!' })
