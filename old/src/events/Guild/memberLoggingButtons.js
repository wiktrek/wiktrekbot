...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!member.moderatable)
      errorArray.push('this member is not moderatable by this bot.');
    ...
    switch (splitArray[1]) {
      case 'Kick':
        {
          member
            .kick(`Kicked by: ${interaction.user.tag} | Member LoggingSystem`)
            .then(() => {
              interaction.reply({
                embeds: [
                  Embed.setDescription(`${member} has been kicked.`).setColor(
                    'Green'
                  ),
                ],
              });
      ...
      case 'Ban':
        {
          member
            .ban(`Banned by: ${interaction.user.tag} | Member LoggingSystem`)
            .then(() => {
              interaction.reply({
                embeds: [
                  Embed.setDescription(`${member} has been banned.`).setColor(
                    'Green'
                  ),
                ],
              });
